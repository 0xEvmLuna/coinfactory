...
<template>
  <el-form :model="formData" ref="vForm" :rules="rules" label-position="top" label-width="27px" size="small"
    @submit.prevent>
    <el-form-item label="设置要杀区块的数量" label-width="0px" prop="input98848">
      <el-input v-model="formData.input98848" size="small" type="text" clearable :minlength="60"
        :maxlength="45"></el-input>
    </el-form-item>
    <div class="static-content-item" @click="resetForm">
      <el-button>取消</el-button>
    </div>
    <div class="static-content-item" @click="submitForm">
    ...
    setup() {
      ...
      const submitForm = () => {
        instance.ctx.$refs['vForm'].validate(async (valid) => {
          ...
          //TODO: 提交表单
          // 获取杀区块的输入数量
          const killBlockAmounts = state.formData.input98848;
          console.log("result", killBlockAmounts)
          ...
          // 合约调用
          // 杀区块
          const txHash = contract.methods.setKillBlock(killBlockAmounts).send({ from: accounts[0] });
          txHash.then((receipt) => {
            console.log(receipt);
          }).catch((error) => {
            console.error(error);
