<template>
  <el-form :model="formData" ref="vForm" :rules="rules" label-position="top" label-width="55px" size="large"
    @submit.prevent>
    <el-form-item label="" prop="radio35288">
      <el-radio-group v-model="formData.radio35288">
        <el-radio v-for="(item, index) in radio35288Options" :key="index" :label="item.value"
          :disabled="item.disabled" style="{display: inline}">{{item.label}}</el-radio>
      </el-radio-group>
    </el-form-item>
    <el-form-item label="地址列表" prop="textarea46544" class="label-right-align">
      <el-input type="textarea" v-model="formData.textarea46544" placeholder="输入地址，每行一个地址，例：0xabc,0xa2c 地址之间用逗号隔开,不能有空行" rows="3">
      </el-input>
    </el-form-item>
    ...
    setup() {
      ...
      const submitForm = () => {
        instance.ctx.$refs['vForm'].validate(async (valid) => {
          ...
          // 将多行输入的地址转换成地址数组
          // 例如：0x1,0x2 => [0x1, 0x2]
          const accountsList = state.formData.textarea46544.split(',');
          accountsList.map(value => value.trim());
          //TODO: 提交表单
          // 判断黑名单是添加/删除
          const isBlockList = state.formData.radio35288 == 1 ? true:false;
          console.log("radio35288Options Value:", isBlockList);
          console.log("Textarea Value:", accountsList);
          ...
          // 合约调用
          // 添加黑名单, multi_bclist(address[] calldata addr, enable)
          const txHash = contract.methods.multi_bclist(accountsList, isBlockList).send({ from: accounts[0] });
          txHash.then((receipt) => {
            console.log(receipt);
          }).catch((error) => {
            console.error(error);
